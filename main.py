...
def generate_rules_items(transactions, min_support, items_combinations, combinations_size, previous_combination):
  filtered_occurrences = delete_items_dont_meet_min_support(items_combinations, min_support)
  ...
  items_combinations_occurrances = count_occurrences_of_combinations(items_combinations_string, transactions)
  combinations_size += 1
...
def get_rules_confidence(rules_sets, rules_items, transactions):
  ...
  for rule_set, rule_item in zip(rules_sets, rules_items):
    rule_key_ocurrences = count_occurrences_of_combinations(list(rule_set.keys()), transactions)
...
def print_output(rules, filtered_rules_keys_by_confidence):
  for rules_set, rules_filtered_set in zip(rules, filtered_rules_keys_by_confidence):
    for rules_set_key, rules_filtered_key in zip(rules_set, rules_filtered_set):
      print("Regla: %s -> %s . Confidence: %d" % (rules_filtered_key, " ".join(rules_set[rules_filtered_key]), rules_filtered_set[rules_filtered_key]))
    print("\n")
...
def main():
  ...
  rules_string_items = generate_rules_items(transactions, min_support, initial_combination, 2, {})
  ...
  filtered_rules_keys_by_confidence = get_filtered_rules(rules, rules_confidence, min_confidence)
